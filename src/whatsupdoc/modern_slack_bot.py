...
from .modern_gemini_rag import ModernGeminiRAGService, RAGResponse
from .modern_vertex_rag_client import ModernVertexRAGClient, SearchResult
...
class ModernSlackBot:
    ...
    def __init__(self) -> None:
        ...
        # Use AsyncApp for proper async support
        self.app = AsyncApp(
        ...
        self.search_client = ModernVertexRAGClient(
        ...
        self.rag_service: Optional[ModernGeminiRAGService] = None
        if self.config.enable_rag_generation:
            self.rag_service = ModernGeminiRAGService(
                project_id=self.config.project_id,
                location=self.config.location,
    ...
    def _setup_handlers(self) -> None:
        """Set up modern async event handlers."""

        @self.app.event("app_mention")
        async def handle_mention(event, say, client):
            await self._handle_query_async(event, say, client, event["text"])

        @self.app.command("/ask")
        async def handle_ask_command(ack, respond, command, client):
            await ack()  # Acknowledge immediately
            await self._handle_query_async(command, respond, client, command["text"])

        @self.app.message("")
        async def handle_dm(message, say, client):
            if message.get("channel_type") == "im":
                await self._handle_query_async(message, say, client, message["text"])
    ...
    async def _handle_query_async(
        ...
        loading_response = await respond_func({
    ...
    async def start_async(self) -> None:
        """Modern async startup with proper resource management."""
        # Test connections
        ...
        if port:
            ...
            # Use async Flask (requires Flask 2.0+)
            await flask_app.run_async(host="0.0.0.0", port=int(port))
        else:
